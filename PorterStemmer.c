...
#include "vdin_str.h"       /* for isupper, islower, tolower */
...
vdin_str stemstring(vdin_str strings){
    ...
    stemmed = vdin_str_crea();

    tam = vdin_str_tama(strings);

    for(i = 0; i < tam; i++){

        s = (char *) malloc(i_max+1);
        ptr = vdin_str_obtiene(strings, i);
        j = strlen(ptr);
        s = strndup(ptr, j);

        s[stem(s,0,j-1)+1] = 0;

        vdin_str_aumd(stemmed, s);
